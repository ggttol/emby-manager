...
NAS: /volume1/docker/clouddrive2/CloudNAS/CloudDrive
  ...
  └─ 整理工具：可按需在 NAS 主机侧写入，但要控制并发
```

实践建议：

- NAS 主机侧的 CloudDrive2 挂载可以是读写，方便整理脚本落文件/改名；但 Emby 容器里仍建议 `read_only: true`。
...
- 115/CloudDrive2 对密集读取很敏感，批量生成 strm、复制 metadata 等任务尽量串行或限速，不要在挂载目录上并发跑大量 ffprobe/下载。
...
在 input/textarea 输入时整套快捷键自动屏蔽,只允许 Esc blur 输入框。找资源 / 智能清理 / 定时等低频 tab 从 nav 或全局搜索进入。
