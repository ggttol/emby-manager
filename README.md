...
**v3.0** 升级了体验:全局 UI 组件库(Modal/Toast/TaskCenter/Drawer/VirtualList/Combobox),危险操作走 Modal + 打字防误删,长任务进 TaskCenter(顶部进度条 + 🔔 bell + 可取消、跨 tab 不丢、刷新自动恢复),键盘快捷键全覆盖,暗/亮三态主题,撤销系统接入 UI,配置导出/导入。后端能力 100% 暴露,11 lib 模块零循环依赖,289 单元测试。
...
- **测试覆盖:** 289 case 覆盖 path 安全 / cfg migrate / TMDb 解析 / c115 内部 / 密码 / HTTP handler / 配置导入导出 / 配置落盘回滚 / strm 列表 / qscore / XFF / scheduler / 任务队列 / 挂载探针 / 持久化日志。**Emby/115 实活 HTTP 端到端未覆盖**(需起 Emby mock,价值低)。
